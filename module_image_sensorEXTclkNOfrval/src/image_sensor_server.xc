...
static inline void generate_sysclk(struct image_sensor_ports &imgports){

    // Note: Delay introduced in app should be removed for non integer clock frequencies.
    configure_clock_rate_at_most(imgports.clk2,100,6); //  8 (12.5MHz) and 6 (16.6MHz). 4 (25MHz and 10 (10MHz).
    configure_out_port(imgports.sys_clk, imgports.clk2, 0);
    configure_port_clock_output(imgports.sys_clk, imgports.clk2);
    start_clock(imgports.clk2);
/*
    set_clock_div(imgports.clk2,4); //  2 (25 MHz), 4 (12.5 MHz).
    set_port_clock(imgports.sys_clk, imgports.clk2);
    set_port_mode_clock(imgports.sys_clk);
    start_clock(imgports.clk2);
*/
}
...
static unsigned config_registers(struct image_sensor_ports &imgports, unsigned height, unsigned width){
    unsigned char i2c_data[2];
...
void image_sensor_server(struct image_sensor_ports &imgports, streaming chanend c_imgSensor){
    ...
    while (1){
        ...
        if (cmd==CONFIG) {
                ...
                horBlank = config_registers(imgports,height,width);

        } else {

                unsigned n_data = height*width/4;
                unsigned i=0, frDetected=0;
                unsigned x;
                ...
                while (!frDetected){
                    ...
                    unsigned run = 1, count = 0;

                    imgports.line_valid when pinseq(0) :> void;
                    imgports.pix_clk :> x;

                    do {
                            imgports.pix_clk when pinsneq(x) :> x;
                            count++;
                    } while (count <= 2*horBlank);

                    imgports.line_valid :> x;
                    if (!x) frDetected = 1;
                }
